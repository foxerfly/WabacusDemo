...
create table tbl_detailinfo(
	...
	county		varchar(30),
	marriage	int,
	interest	varchar(100),--兴趣爱好
	photo		varchar(100),--照片
	description	text,
	jl		varchar(100),
	orderline	int
);
...
insert into tbl_zhaoping values('网络工程师',5030,1,'本科以上');
insert into tbl_zhaoping values('软件工程师',6800,3,'本科以上');
insert into tbl_zhaoping values('高级会计师',8000,1,'硕士以上');
insert into tbl_zhaoping values('计算机运行维护工程师',4500,2,'专科以上');
insert into tbl_zhaoping values('策划经理',8200,1,'本科以上');
insert into tbl_zhaoping values('质量管理工程师',5500,5,'本科以上');
insert into tbl_zhaoping values('普工',1200,20,'无');
insert into tbl_zhaoping values('厨师',3000,2,'无');
insert into tbl_zhaoping values('项目经理',8500,2,'本科以上');
...
CREATE  PROCEDURE sp_testInsertData(
    ...
    @out_rtnValue varchar(50) output
)
AS
    ...
    set @out_rtnValue='添加工号：'+@in_deptno+'，姓名：'+@in_deptname+'的记录成功';
...
AS
    ...
    set @out_rtnValue='修改工号：'+@in_deptno_old+'的记录成功';
...
AS
    ...
    if @txtno<>'' set @var_sql=@var_sql+' and A.no like  ''%'+@txtno+'%''';
    if @txtprovince<>'' set @var_sql=@var_sql+' and province like  ''%'+@txtprovince+'%''';
    if @txtage<>'' set @var_sql=@var_sql+' and A.age >='+@txtage;
    exec SP_WABACUS_EXECUTE @var_sql,'no,name desc',@i_systeminfo;
